...
namespace robot_calibration
{
                          ...
                          bool progress_to_stdout)
  {
    ...
    for (size_t i = 0; i < params.models.size(); ++i)
    {
      ...
      {
        RCLCPP_INFO_STREAM(logger, "Creating chain '" << params.models[i].name << "' from " << params.base_link << " to " << params.models[i].frame);
      ...
      {
        RCLCPP_INFO_STREAM(logger, "Creating camera3d '" << params.models[i].name << "' in frame " << params.models[i].frame);
    ...
    for (size_t i = 0; i < params.free_frames_initial_values.size(); ++i)
    {
      ...
      {
        RCLCPP_ERROR_STREAM(logger, "Error setting initial value for " << params.free_frames_initial_values[i].name);
      }
    }
